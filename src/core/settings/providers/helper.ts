...
export class CoreSettingsHelper {
   ...
   */
  async deleteSiteStorage(
    siteName: string,
    siteId: string
  ): Promise<CoreSiteSpaceUsage> {
    const siteInfo: CoreSiteSpaceUsage = {
      cacheEntries: 0,
      spaceUsage: 0,
    };

    siteName = await this.filterProvider.formatText(
      siteName,
    ...
    const message = this.translate.instant("core.settings.deletesitefiles", {
      sitename: siteName,
    });
      ...
      site
        ...
        .catch(async (error) => {
          if (error && error.code === FileError.NOT_FOUND_ERR) {
            ...
            this.domUtils.showErrorModal(
              "core.settings.errordeletesitefiles",
              true
            );
        ...
        .then(async () => {
          this.eventsProvider.trigger(
            CoreEventsProvider.SITE_STORAGE_DELETED,
            {},
            site.getId()
          );
  ...
  async getSiteSpaceUsage(siteId?: string): Promise<CoreSiteSpaceUsage> {
    ...
    await Promise.all([
      this.calcSiteClearRows(site).then(
        (rows) => (siteInfo.cacheEntries = rows)
      ),
  ...
  protected async calcSiteClearRows(site: CoreSite): Promise<number> {
    ...
    await Promise.all(
      clearTables.map(
        async (name) =>
          (totalEntries =
            (await site.getDb().countRecords(name)) + totalEntries)
      )
    );
  ...
  synchronizeSite(syncOnlyOnWifi: boolean, siteId: string): Promise<any> {
    ...
    if (hasSyncHandlers && !this.appProvider.isOnline()) {
      ...
      return Promise.reject(
        this.translate.instant("core.settings.cannotsyncoffline")
      );
    } else if (
      hasSyncHandlers &&
      syncOnlyOnWifi &&
      this.appProvider.isNetworkAccessLimited()
    ) {
      return Promise.reject(
        this.translate.instant("core.settings.cannotsyncwithoutwifi")
      );
    }

    // Invalidate all the site files so they are re-downloaded.
    promises.push(
      this.filePoolProvider.invalidateAllFiles(siteId).catch(() => {
        // Ignore errors.
      })
    );
    ...
    promises.push(
      this.sitesProvider.getSite(siteId).then((site) => {
        ...
        return site.invalidateWsCache().then(() => {
          ...
          subPromises.push(
            site.checkIfLocalMobileInstalledAndNotUsed().then(
              () => {
                // Local mobile was added. Throw invalid session to force reconnect and create a new token.
                this.eventsProvider.trigger(
                  CoreEventsProvider.SESSION_EXPIRED,
                  {},
                  siteId
                );
  ...
  initDomSettings(): void {
    ...
    this.configProvider
      .get(
        ...
        CoreConfigConstants.font_sizes[0].toString()
      )
      .then((fontSize) => {
  ...
  initColorScheme(): void {
    if (!!CoreConfigConstants.forceColorScheme) {
      ...
      this.configProvider
        .get(CoreConstants.SETTINGS_COLOR_SCHEME, defaultColorScheme)
        .then((scheme) => {
  ...
  isColorSchemeDisabledInSite(site?: CoreSite): boolean {
    ...
    return site ? site.isFeatureDisabled("NoDelegate_") : false;
  }
